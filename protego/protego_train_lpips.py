...
def train_protego_mask_lpips(cfgs: OmegaConf, 
                            frs: List[FR], 
                            train_dl: DataLoader, 
    ...
    percep_loss_fn = lpips.LPIPS(net='vgg', verbose=False).to(device)
    # The training loop
    for epoch in range(epoch_num):
        ...
        for batch_idx, tensors in pbar:
            ...
            normalized_prot_face = (protected_faces - 0.5) * 2
            normalized_orig_face = (orig_faces - 0.5) * 2
            percep_loss = percep_loss_fn(normalized_prot_face, normalized_orig_face).mean()
            percep_term = percep_loss
            ...
            # Record the loss
            hyper_loss = _hyper_loss.item() / len(frs) 
