...
class Renderer(nn.Module):
    def __init__(self, render_full_head=False, obj_filename='assets/head_template.obj'):
        ...
        print(f"raw_uvcoords shape: {uvcoords.shape} and range: {uvcoords.min()} - {uvcoords.max()}")

        # uv coords
        uvcoords = torch.cat([uvcoords, uvcoords[:,:,0:1]*0.+1.], -1) #[bz, ntv, 3]
        print(f"uvcoords shape: {uvcoords.shape} and range: {uvcoords.min()} - {uvcoords.max()}")
        uvcoords = uvcoords*2 - 1; uvcoords[...,1] = -uvcoords[...,1]
        print(f"uvcoords shape: {uvcoords.shape} and range: {uvcoords.min()} - {uvcoords.max()}")
        face_uvcoords = face_vertices(uvcoords, uvfaces)
        print(f"face_uvcoords shape: {face_uvcoords.shape} and range: {face_uvcoords.min()} - {face_uvcoords.max()}")
        #print(face_uvcoords[0][0][0])
    ...
    def rasterize(self, vertices, faces, attributes=None, h=None, w=None):
        ...
        print(f"bary_coords shape: {bary_coords.shape}, range: {bary_coords.min()} - {bary_coords.max()}")
