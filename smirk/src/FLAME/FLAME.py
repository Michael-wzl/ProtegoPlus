...
class FLAME(nn.Module):
    ...
    def __init__(self, flame_model_path='assets/FLAME2020/generic_model.pkl', 
                 flame_lmk_embedding_path='assets/landmark_embedding.npy', n_shape=300, n_exp=50, base_path=None):
        flame_model_path = flame_model_path if base_path is None else os.path.join(base_path, flame_model_path)
        flame_lmk_embedding_path = flame_lmk_embedding_path if base_path is None else os.path.join(base_path, flame_lmk_embedding_path)
        ...
        l_eyelid_path = f'{base_path}/assets/l_eyelid.npy' if base_path else 'assets/l_eyelid.npy'
        r_eyelid_path = f'{base_path}/assets/r_eyelid.npy' if base_path else 'assets/r_eyelid.npy'
        self.register_buffer('l_eyelid', torch.from_numpy(np.load(l_eyelid_path)).to(self.dtype)[None])
        self.register_buffer('r_eyelid', torch.from_numpy(np.load(r_eyelid_path)).to(self.dtype)[None])
        ...
        lmk_embeddings_mp_path = f"{base_path}/assets/mediapipe_landmark_embedding/mediapipe_landmark_embedding.npz" if base_path else "assets/mediapipe_landmark_embedding/mediapipe_landmark_embedding.npz"
        lmk_embeddings_mp = np.load(lmk_embeddings_mp_path)
